<?php
/*
Plugin Name: La Liga Futbol Football Rankings Lite
...
function LALIGASPRLPageURL() {
 $LALIGASPRLpageURL = 'http';
 $LALIGASPRLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $LALIGASPRLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $LALIGASPRLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $LALIGASPRLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_laligasprlstats() 
{
?>
<h2>La Liga Rankings Lite</h2>
<?php laligasprl_stats(); ?>
<?php
}

function laligasprlstats_install()
{
register_sidebar_widget(__('La Liga Rankings Lite'), 'widget_laligasprlstats'); 
}
add_action("plugins_loaded", "laligasprlstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'laligasprl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'laligasprl_stats_remove' );

function laligasprl_stats_install() 
{
...
add_option("laligasprl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("laligasprl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("laligasprl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE LALIGA Power Rankings Installation", "Hi\n\nLITE LALIGA Power Rankings Activated at \n\n".LALIGASPRLPageURL()."\n\nLALIGA Power Rankings Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function laligasprl_stats_remove() 
{
/* Deletes the database field */
delete_option('laligasprl_scroll_text_color');
delete_option('laligasprl_scroll_text_color1');
delete_option('laligasprl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'laligasprl_stats_admin_menu');

function laligasprl_stats_admin_menu() {

add_options_page('La Liga Rankings Lite', 'La Liga Rankings Lite Settings', 'administrator', 'laliga-power-rankings-lite.php', 'laligasprl_stats_plugin_page');
}
}
function laligasprl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('laligasprl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('laligasprl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('laligasprl_scroll_text_color2'));

$myads = "http://www.ibet.ws/la_liga_stats_rankings_magpie_lite/la_liga_stats_rankings_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $myads;
$rss = laligasprl_fetch_rss( $url );
...
function laligasprl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
	...
	?>
   <div>
   <h2>La Liga Rankings Options Page</h2>
...
<p>
<?php echo laligasprl_stats(); ?>
</p>
<?php echo $myads; ?>
<!-- Start Advanced Plugins List -->
  <h2>If You Want MORE Stats and Information:</h2>
  <p>A93D Offers FREE upgrades for this stats package, that allow you to display advanced and more complete LALIGA Power Rankings.
  ...
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/la-liga-rankings.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/la-liga-rankings.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
...
<p><strong>Soccer / Football / Futbol</strong><br />
  <strong>Premier League rankings</strong> <a href="http://wplatest5.thoseamazingparks.com">Preview</a> | <a href="http://www.ibet.ws/download/la-liga-rankings.zip">Download Now</a> - Complete Premier League Rankings for all 20 Teams.</p>
